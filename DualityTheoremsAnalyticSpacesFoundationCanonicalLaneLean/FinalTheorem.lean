...
import DualityTheoremsAnalyticSpacesFoundationCanonicalLaneLean.GateLemmas
...
def NativeClosureTheorem (A : AdmissibleClass) : Prop :=
  ConstrainedDualityClosure A

def ConditionalRouteClosure (A : AdmissibleClass) : Prop :=
  ConstrainedDualityClosure A ∧ ClassicalSourceBoundaryCarried
...
theorem native_closure_theorem_checked (A : AdmissibleClass) :
  ...
  exact constrained_duality_endgame A

theorem conditional_route_closure_checked (A : AdmissibleClass) :
    ConditionalRouteClosure A := by
  exact And.intro (constrained_duality_endgame A) classical_source_boundary_carried_checked
